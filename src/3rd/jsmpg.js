(function(window) {
    ...
    var jsmpeg = window.jsmpeg = function(url, opts) {
        ...
        this.textcanvas = opts.textcanvas || document.createElement('canvas');
    ...
    jsmpeg.prototype.updateLoaderGL = function(ev) {
        ...
        gl.drawArrays(gl.TRIANGLES, 0, this.VtxArray.length / 2);
    };
    ...
    jsmpeg.prototype.initBuffers = function() {
        ...
        this.resizeCanvas(this.width/4, this.height/4);
    ...
    //
    //  resize Canvas function made by Justin-Chang - 10401020
    //
    ...
    jsmpeg.prototype.YCbCrToRGBA = function() {
		...
		var pRGBA  = new Int32Array(this.width * this.height * 4.0);
		if (this.currentRGBA) {
			pRGBA = this.currentRGBA.data;
		}         
    ...
    jsmpeg.prototype.cartMatrix = GetEye4();
    jsmpeg.prototype.mvMatrix = GetTranslateMatrix(0, 0, 0);
    jsmpeg.prototype.pjMatrix = GetEye4();
	jsmpeg.prototype.pjFov = 45.0;
    jsmpeg.prototype.viewMatrix = makeLookAtMatrix(
		0.5, -0.5, GetCorrectedZunit(45),
		0.5, -0.5, 0.0,
		0.0,  1.0, 0.0
	); 
	jsmpeg.prototype.canvastext = "Hello PI-view!";
	jsmpeg.prototype.NmlVtx = new Float32Array([
		0, 0,
	   	1, 0,
	   	0, 1,
	   	0, 1,
	   	1, 0, 
		1, 1]);
	jsmpeg.prototype.VtxArray = new Float32Array([
		0, 0,
		   480, 0,
		   0, 480,
		   0, 480,
		   480, 0,
		480, 480]);
	jsmpeg.prototype.TexCordArray = new Float32Array([
		0, 0,
		   1, 0,
		   0, 1,
		   0, 1,
		   1, 0,
		1, 1]);
   /*  jsmpeg.prototype.VtxArray = new Float32Array(vtx_buffer); */
    /* jsmpeg.prototype.TexCordArray = new Float32Array(txt_buffer); */
    ...
    jsmpeg.prototype.initWebGL = function() {
	    ...
	    this.initYCBCRTextures();	
		this.initglBuffers();
		this.initLoadShaderPorgram();
      
        return true;
    };

	jsmpeg.prototype.initShaderProgram = function() {
		var gl = this.gl;
	...
	jsmpeg.prototype.initYCBCRTextures = function() {
		// setup textures
	...
	jsmpeg.prototype.initglBuffers = function() {
		var gl = this.gl;
		...
		this.vertexAttr = gl.getAttribLocation(this.program, 'vertexPos');
		...
		gl.bufferData(gl.ARRAY_BUFFER, this.TexCordArray, gl.STATIC_DRAW);		

		this.texAttr = gl.getAttribLocation(this.program, 'texturePos');
		gl.enableVertexAttribArray(this.texAttr);
		gl.vertexAttribPointer(this.texAttr, 2, gl.FLOAT, false, 0, 0);
	};

	jsmpeg.prototype.initLoadShaderPorgram = function() {
		var gl = this.gl;
        ...
        var vertexAttr = gl.getAttribLocation(this.loadingProgram, 'vertexPos');
    ...
    jsmpeg.prototype.renderFrameGL = function() {
        var gl = this.gl;
		console.log("%f, %f",vtx_buffer.length/2, txt_buffer.length/2);
		...
		var cart_width = this.canvas.width;
		var cart_height = this.canvas.height;
		var cart_vtx = GetCartesianVtx(cart_width, cart_height);

		this.pjMatrix = makePerspective(this.pjFov, 1.0, 0.1, 500.0);
        this.setPjMatrix(this.pjMatrix);

		this.setViewMatrix(this.viewMatrix);

		this.cartMatrix = GetCartesianMatrix(cart_width, cart_height,1);
        this.setCartMatrix(this.cartMatrix);
        this.setMvMatrix(this.mvMatrix);
	
		this.setVtxBuffer(this.VtxArray);
        this.setTexCordBuffer(this.TexCordArray);


		console.log(this.width * this.height / 2.0);
        // Start rendering
        this.TextCanvasRenderer.drawText(this.canvastext);
        gl.drawArrays(gl.TRIANGLES, 0, this.VtxArray.length / 2.0);
    };
    ...
    //	- by Justin Chang
    //
    jsmpeg.prototype.setVtxBuffer = function(vtx_array) {
		if (arguments.length === 0) {
			var cart_width = this.canvas.width;
			var cart_height = this.canvas.height;
			var cart_vtx = GetCartesianVtx(cart_width, cart_height);

            this.VtxArray = cart_vtx;
        } else {
            this.VtxArray= vtx_array;
        }
		var gl = this.gl;
    ...
    jsmpeg.prototype.setTexCordBuffer = function(txt_array) {
		if (arguments.length === 0) {
            this.TexCordArray = this.NmlVtx;
        } else {
            this.TexCordArray = txt_array;
        }
        var gl = this.gl;
    ...
    //
    // Transform matrix from homogenouse to Cartesian coordiante 
    // - made by Jusitn-Chang
    //
    jsmpeg.prototype.setCartMatrix = function(matrix) {
        if (arguments.length === 0) {
            this.cartMatrix = GetEye4();
		} else {
            this.cartMatrix = matrix;
        }
        var gl = this.gl;
        var shaderProgram = this.program;

        var cartUniform = gl.getUniformLocation(shaderProgram, "homo2Cart");
        gl.uniformMatrix4fv(cartUniform, false, new Float32Array(this.cartMatrix));
    ...
    //
    jsmpeg.prototype.setMvMatrix = function(matrix) {
        if (arguments.length === 0) {
            this.mvMatrix = GetEye4();
        } else {
        ...
        gl.uniformMatrix4fv(mvUniform, false, new Float32Array(this.mvMatrix));
    ...
    //	- by Justin Chang
    //
    jsmpeg.prototype.setPjMatrix = function(matrix) {
        if (arguments.length === 0) {
            this.pjMatrix = GetEye4();
        } else {
            this.pjMatrix = matrix;
        }
        ...
        var mvUniform = gl.getUniformLocation(shaderProgram, "uPMatrix");
        gl.uniformMatrix4fv(mvUniform, false, new Float32Array(this.pjMatrix));
    ...
    //	- by Justin Chang
    //
    jsmpeg.prototype.setViewMatrix = function(matrix) {

        if (arguments.length === 0) {
            this.viewMatrix = makeLookAtMatrix(
			0.5, -0.5, GetCorrectedZunit(this.pjFov),
			0.5, -0.5, 0.0,
			0.0,  1.0, 0.0
			);
        } else {
        ...
        var mvUniform = gl.getUniformLocation(shaderProgram, "uVMatrix");
        gl.uniformMatrix4fv(mvUniform, false, new Float32Array(this.viewMatrix));
    ...
    var
        ...
        SHADER_VERTEX_IDENTITY = [
            ...
            'void main() {',
            'gl_Position = uPMatrix * uVMatrix * uMVMatrix * homo2Cart * vec4((vertexPos) * vec2(1, -1), 0.0, 1.0);',
			'texCoord = texturePos;',
            '}'
        ].join('\n');

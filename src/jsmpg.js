...
jsmpeg.prototype.mvMatrix = GetEye4();
jsmpeg.prototype.pjMatrix = GetEye4();
jsmpeg.prototype.viewMatrix = GetEye4();
jsmpeg.prototype.canvastext = "Hello PI-view!";
jsmpeg.prototype.VtxArray = new Float32Array([0, 0, 0, 1, 1, 0, 1, 0, 0, 1, 1, 1]);
jsmpeg.prototype.TexCordArray =new Float32Array([0, 0, 0, 1, 1, 0, 1, 0, 0, 1, 1, 1]);
...
jsmpeg.prototype.setMvMatrix = function() {
	var gl = this.gl;
...
jsmpeg.prototype.setPjMatrix = function() {
	var gl = this.gl;
	var shaderProgram = this.program;
...
jsmpeg.prototype.setViewMatrix = function() {
	var gl = this.gl;
	var shaderProgram = this.program;
...
jsmpeg.prototype.setTexCordBuffer = function() {
	var gl = this.gl;
	var vertexAttr = gl.getAttribLocation(this.program, 'vertex');
	gl.enableVertexAttribArray(vertexAttr);
	gl.vertexAttribPointer(vertexAttr, 2, gl.FLOAT, false, 0, 0);
	gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(this.TexCordArray), gl.STATIC_DRAW);
};
...
var
	...
	SHADER_VERTEX_IDENTITY = [
		...
		'void main() {',
			'texCoord = vertex;',
			'gl_Position = uVMatrix * uPMatrix * uMVMatrix * vec4((vertex * 2.0 - 1.0) * vec2(1, -1), 0.0, 1.0);',
		'}'
	].join('\n');
